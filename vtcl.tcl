...
proc vTcl:load_widgets {} {
    ...
    #
    # Query for the list of libs to load.
    # Either from prefs or defaults to all available libraries.
    #
...
#
# Get a locally provided 'tablelist' module
#
proc ::vTcl::load_tablelist {} {
    vTcl:splash_status "Loading Tablelist"

    uplevel #0 {
    	set dir [file join $vTcl(LIB_DIR) tablelist]
	source  [file join $dir pkgIndex.tcl]
    }
...
#    set vTcl(version)   1.60	# Clamped off,2016-02-19/hph
    ...
    #   M.R.m(.p)
    #
    # i.e.: M, R, m & p can be any number of digits
    # but at least 3 positions must be present.
    # M cannot be '0',
    # An optional 4th field 'p' denotes a patchlevel
    # not for production; such a version lives on GitHub
    # i.e. 
    #   - for major vTcl version 9 is written: 9.0.0
    #   - for an upcoming version 8.7 it is 8.7.0
    #     (a.s.o.)
    #	- a version 8.6.3.65 is a patchlevel
    #
    # Attn.: 	This must match with the regex in lib/file.tcl!
    # 		Or vTcl will mourn on load with:
    # 		  "This ist not a vTcl project"
    #-
    set vTcl(version)   8.6.0.5
...
proc vTcl:setup_gui {} {
    ...
    ##
    ## We use our own version of Bwidgets with some bug fixes. 
    ## Will submit them the bugs when time permits. (C.Cavin/somewhere ~2000)
    ##
    package require -exact BWidget 1.3.1
    package require -exact Tablelist 5.15
...
proc vTcl:main {argc argv} {
    ...
    if {$tcl_version < 8.3} {
	wm deiconify .
	wm title . "Time to upgrade TCL/TK"
